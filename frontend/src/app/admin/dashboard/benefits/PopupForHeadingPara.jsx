        ...
        <div>
            ...
            <div className="flex mt-4 flex-col md:flex-row items-center justify-between bg-white/5 border border-white/10 p-6 rounded-3xl mb-10 backdrop-blur-md gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">{ParaAndTitleOfCards?.data?.o}</p>
                        <h1 className="text-2xl font-black tracking-tight text-white"> <span className="text-indigo-500">{ParaAndTitleOfCards?.data?.p}</span></h1>
                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">{ParaAndTitleOfCards?.data?.q}</p>
                    </div>
                </div>
            ...
            <div className='fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md p-4'>
                <div className='bg-[#1e293b] w-full max-w-2xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden'>
                    ...
                    <form className='p-6 space-y-5'>
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.o || ""}</label>
                            <input
                                type="text"
                                value={payload.o}
                                onChange={(e) =>
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.p || ""}</label>
                            <input
                                type="text"
                                value={payload.p}
                                onChange={(e) =>
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.q || ""}</label>
                            <textarea 
                                value={payload.q}
                                onChange={(e) =>
                                    setpayload(prev => ({
