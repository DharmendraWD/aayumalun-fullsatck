...
// console.log(ParaAndTitleOfCards)
        ...
        <div>
            ...
            <div className="flex mt-4 flex-col md:flex-row items-center justify-between bg-white/5 border border-white/10 p-6 rounded-3xl mb-10 backdrop-blur-md gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">{ParaAndTitleOfCards?.data?.d || ""}</p>
                        <h1 className="text-2xl font-black tracking-tight text-white"> <span className="text-indigo-500">{ParaAndTitleOfCards?.data?.c || ""}</span></h1>
                        <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">{ParaAndTitleOfCards?.data?.e || ""}</p>
                    </div>
                </div>
            ...
            <div className='fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-md p-4'>
                <div className='bg-[#1e293b] w-full max-w-2xl rounded-3xl border border-white/10 shadow-2xl overflow-hidden'>
                    <div className='flex justify-between items-center p-6 border-b border-white/5'>
                        <h2 className='text-xl font-bold flex items-center gap-2'><MdAdd className='text-blue-500' /> Edit Heading And Para Above Cards</h2>
                    ...
                    <form className='p-6 space-y-5'>
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.c}</label>
                            <input
                                type="text"
                                value={payload.c}
                                onChange={(e) =>
                                ...
                                className="w-full p-4 bg-slate-900/50 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all min-h-[40px]"
                            />
                        </div>
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.d}</label>
                            <input
                                type="text"
                                value={payload.d}
                                onChange={(e) =>
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-slate-400 mb-2">{payload.e}</label>
                            <textarea 
                                value={payload.e}
                                onChange={(e) =>
                                    setpayload(prev => ({
                                ...
                                placeholder="Provide a detailed ans..."
                                className="w-full p-4 bg-slate-900/50 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none transition-all min-h-[140px]"
                            />
                        </div>
