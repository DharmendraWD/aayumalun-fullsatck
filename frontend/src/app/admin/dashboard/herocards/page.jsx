...
const page = async () => {
  ...
  try {
    const response = await fetch(`${BASE_API}/contents/hero/heroCards`, {
    });
            ...
            gallaryData.data.map((client, index) => (
              ...
              >
                  ...
                  <div className="flex relative flex-col cardBg m-2.5 overflow-hidden rounded-md h-80">
                    ...
                    <p className="text-sm px-4 mt-2 text-gray-300 ">
                     First Title  :{client.title1}
                    </p>
                    <p className="text-sm px-4 mt-2 text-gray-300 ">
                     Second Title  :{client.title2}
                    </p>
                    <img
                      src={`${BASE_CONTENT}/uploads/heroCards/${client.image}`}
                      alt={`gallery-${index}`}
                      className="w-[300px] mx-auto object-cover rounded-md"
                    />
                    <p className="text-sm min-h-[50px] px-4 mt-2 text-gray-300 ">
                     Short Description :{client.description}
                    </p>
                  </div>
